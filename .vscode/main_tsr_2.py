...
# Bring up CAN interface if enabled in config
...
oled.display_centered_text("Welcome to\nDiagnostics")
time.sleep(2)
    ...
    while True:
        ...
        if GPIO.input(BTN_ENTER) == GPIO.LOW:
            ...
            if selected_option == "ECU Information":
                ...
                time.sleep(1)
                GPIO.cleanup()
                if config.get("can_interface", {}).get("shutdown_on_exit", False):
